...
- **Verify reviewer comments before merge**: Extends **Verify CI green before merge**. CI green is not sufficient — AI reviewers (`Copilot`, `sentry[bot]`) and human reviewers emit line-level comments that do not fail CI but can hide real bugs. Before `gh pr merge`, enumerate line comments with `gh api repos/:owner/:repo/pulls/<N>/comments --jq '.[] | {user: .user.login, path, line, start_line, original_line, commit: .commit_id, body}'`; `line: null` means the comment is anchored to a multi-line range (read `start_line`) or to an outdated commit (cross-reference `commit` against `gh pr view <N> --json headRefOid`). Bug predictions from `Copilot` and `sentry[bot]` live in `pulls/<N>/comments` — `gh pr view <N> --json reviews,reviewDecision` surfaces only review summaries (often with empty `body` for bots), and `reviewDecision` can be empty with real findings present, so do not rely on review state alone. Confirm each comment is (a) addressed in a follow-up commit, (b) already marked resolved by the bot (e.g. sentry prefixes `*Resolved in <sha>`), or (c) explicitly dismissed with a reply stating the reason. See also `docs/governance.md` § "PR レビューとマージ".
