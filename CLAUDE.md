...
- **Verify reviewer comments before merge**: Extends **Verify CI green before merge**. CI green does not cover line-level comments from AI reviewers (`Copilot`, `sentry[bot]`) and humans — they do not fail CI but can hide real bugs. Before merge, enumerate every open line comment and dispose of each one (follow-up commit, bot self-resolve, or thread reply with reason). See `docs/governance.md` § "PR レビューとマージ" > "レビュアーコメントの扱い" for the exact `gh api` commands, pagination pitfalls, and disposition rules. See also `skills/midstream/rr-midstream-gh-address-comments-001/SKILL.md` for the reviewee-side workflow.
