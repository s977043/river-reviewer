...
| `findings`  | `array`              | No   | レビュー指摘事項の配列。各要素は `output.schema.json` の issue に準拠。    |
...
| フィールド       | 型       | 説明                                                                                                       |
| ---------------- | -------- | ---------------------------------------------------------------------------------------------------------- |
| `selectedSkills` | `array`  | 実行対象として選択されたスキルの配列。各要素は `id`（必須）、`name`（必須）、`phase`、`modelHint` を持つ。 |
| `skippedSkills`  | `array`  | スキップされたスキルの配列。各要素は `id`（必須）と `reasons`（必須、文字列配列）を持つ。                  |
| `plannerMode`    | `string` | AI プランナーのモード。`off` / `order` / `prune`。                                                         |
| `plannerReasons` | `array`  | プランナーによるスキルごとの判断理由。各要素は `id` と `reason` を持つ。                                   |
...
| `tokenEstimate` | `number` | 最適化後の差分テキストの推定トークン数。       |
