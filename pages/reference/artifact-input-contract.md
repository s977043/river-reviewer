...
2. **設定ファイル経由**（`river review plan` / `river review exec` CLI で定義済み）。`river.config.*` 内の `artifacts` セクション。
...
- GitHub Action の inputs（参考: `runners/github-action/action.yml`）から artifact 指定を渡せるようにする（未実装、別途対応予定）。
