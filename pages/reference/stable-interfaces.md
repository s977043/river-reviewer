...
- `river skills <path>`: skill ベースのレビューを実行する（`skills import` / `skills export` / `skills list` で Agent Skills と相互変換）
- `river review plan`: 上流アーティファクトを解決して Review Artifact を出力する（[仕様](./cli-review-plan-spec.md)）
- `river runs list|diff <id> <id>|summary`: 保存済みレビュー実行（`.river/runs/`）の一覧・差分・集計を表示する
...
- `--output <text|markdown|json|yaml>`: 出力形式（GitHub Actions は `markdown` を使用、`yaml` は [YAML 出力](./output-format-yaml.md) を参照）
...
Action は安定動作のため、`@main` ではなく **リリースタグへピン留め**することを推奨します（例: `@v1.2.2`）。
