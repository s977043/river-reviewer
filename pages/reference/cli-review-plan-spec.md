...
>
> PR-2 暫定挙動（実装現況）: 後方互換のため `--output`/`--format` 未指定時は JSON を出力します。`json` 明示は受理、`text`/`markdown` の明示指定は未実装エラー（exit 3）、`yaml` は review 契約外のため exit 3。`text`/`markdown` レンダリングは後続で実装します。
