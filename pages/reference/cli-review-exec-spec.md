...
| `--max-cost <usd>` | number | （無）  | 見積もりが上限を超えた場合に skill 実行を中断する（exit `1`）。`--estimate` と併用可。                                 |
...
- `--max-cost` を超える見積もりとなった（`--estimate` 単独実行時を除く）。
...
- **LLM 呼び出し失敗**: skill 単位にリトライしたうえで失敗を `findings` に記録する。復旧不能な場合は `status: error` とする。
