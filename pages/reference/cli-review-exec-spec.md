...
>
> 追補（[#935](https://github.com/s977043/river-review/pull/935)、2026-05-29）: **`river review exec --plan <file>` は実装済み**。plan ファイルを読み込み `generateReview` 経由で skill を実行し Review Artifact を出力する。`--plan` なしの通常 `exec`（非 dry-run）は引き続き exit 3。`verify` 実行は未実装。
