...
- **manual 次元のスケーラビリティ**: `actionability` は人間の評価が必要であり、大規模な CI 実行ではボトルネックとなる。中期的には LLM-as-a-Judge による代替を検討している。
- **binary の粒度**: `phase_consistency` は binary だが、複数フェーズにまたがるケースでは部分的な一致を表現できない。ratio への移行を検討する余地がある。
