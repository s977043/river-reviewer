...
2. **機械的チェック（heuristic / モデル不要）** — 正規表現などで決定論的に判定できる観点は、**LLM・API キーなしで動作する**。現在の対応はハードコード secret 検出 / GitHub Actions のリスク / silent-catch（例外握り潰し）/ テスト欠落の 4 スキル。機械的に判定できる観点は今後拡張できる。
