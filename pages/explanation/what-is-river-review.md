...
River Review のスキルは、River Review 自身が LLM を呼ぶための設定ではなく、**AI のレビュー能力を強化する capability pack（スキル / エージェント定義）**である。実行のされ方は 3 通りで、**LLM キーが必要かどうかは「実行面」ではなくこのモデルで決まる**。
...
> まとめ: 通常の AI 駆動開発ではエージェントがスキルを適用するため **LLM キーは不要**。LLM キーが要るのは **GitHub Action / standalone CLI のヘッドレス実行**で、かつ機械的チェック以外のスキルを動かす場合だけである。
