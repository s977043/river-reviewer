...
2. **Mechanical checks (heuristic / no model)** — viewpoints that can be decided deterministically (e.g. via regex) **run with no LLM and no key**. Today this covers hardcoded-secret detection / dangerous `eval` & XSS / disabled TLS verification / GitHub Actions risks / silent-catch / leftover `debugger` / missing tests / focused tests (leftover `.only`), handled by the midstream-security and downstream-test skills. More mechanically-decidable viewpoints can be added over time.
