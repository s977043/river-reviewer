...
このページは **AI エージェント（自律 / 半自律のコーディングエージェント）** が River Review を「いつ・どう呼ぶか」をケース別にまとめた実践ガイドです。AI 駆動開発では、コードを書くのも直すのもエージェントですが、**レビューは River にゲートさせ、出力（JSON）を読んで次の行動を機械的に決める**のが要点です。
...
実装に入る**前**に、要件の曖昧さ・設計の検証漏れ・計画の不整合を潰します。
...
- Independent Review Synthesis skill が重複排除・検証を行い、統合 verdict を出します。詳細は [W チェック](./w-check.md) / [Independent Review Synthesis を使う](./use-independent-review-synthesis.md)。
- マルチエージェント開発で、各エージェントのレビューを 1 つに束ねる用途に有効。
...
- ❌ **pre-exec を `--phase upstream` 無しで呼ぶ** → upstream skill がフェーズ不一致で全 skip される。
