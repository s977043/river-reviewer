...
1. このリポジトリを Vercel の別プロジェクト（例: `river-review-docs`）として追加する。
2. Build Command: `npm run build`、Output Directory: `build` を設定する。
3. 環境変数を追加する。
   - `DOCS_SITE_URL=https://<docs-domain>` （例: `https://river-review-docs.vercel.app`）
...
GitHub Pages 向けに `DOCS_BASE_URL` を指定しない場合は、従来どおり `/river-review/` 配信でビルドされます。

## アプリ側（例: `<app-domain>`／`river-review.the3396.com` など）
