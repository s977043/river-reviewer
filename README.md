...
> **注意**: `river review verify` はまだ実装中です (#802 Phase 3)。`river review plan` および `river review exec`（`--plan` replay 含む）は v0.68.0 時点で利用可能です。
...
タグは `@v0.70.0` などのリリースタグにピン留めしてください。浮動タグを使う場合は `@v0` のようなエイリアスタグを用意して運用します（任意）。

<!-- x-release-please-start-version -->

最新リリース: [v0.70.0](https://github.com/s977043/river-reviewer/releases/latest)
...
コンセプト刷新（2026-05）に伴い、Roadmap は以下の 7 Epic で構成しています。状態列は v0.69.0 時点の実装到達点を示します。
...
| **Epic 1**: First-Run Adoption             | npm 配布、`npx river try`、10 分 Quick Start                                                                 | Partial — B1 設計 + dry-run packaging audit が landed ([#886](https://github.com/s977043/river-reviewer/pull/886))。publish ワークフロー + `publishConfig` は実装済み ([#942](https://github.com/s977043/river-reviewer/pull/942))、`NPM_TOKEN` シークレット設定で有効化される ([#800](https://github.com/s977043/river-reviewer/issues/800))。`npx river try` は別途対応予定 |
| **Epic 2**: SDLC Gates                     | `plan` / `exec` / `verify` CLI 安定化、artifact-input-contract v1                                            | Partial — `plan` / `exec` は v0.53.0 で安定（silent-skip 5/6 解消済み）。`exec --plan` replay execution は v0.68.0 でリリース済み（[#935](https://github.com/s977043/river-reviewer/pull/935)）。`verify` 実行は未実装                                                                                                                                                        |
