...
- 変更はタスク単位でブランチを切り、PR を作成する。PR では `npm test` / `npm run lint` を通し、Gemini / Codex へのレビュー依頼とセルフレビューを必須とする。
...
- PR 前に `npm test` / `npm run lint` を実行（必要に応じて `npm run agents:validate` / `npm run skills:validate`）。
