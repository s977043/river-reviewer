# River Review Smoke Test Prompts

River Review のルーティング機能と基本的なレビュー動作を確認するためのプロンプト集。

## 1. 一般的なコードレビュー (Default Fallback)

**目的**: 特定の専門領域に該当しない場合、デフォルト（一般コードレビュー）へフォールバックするか確認。
※ `river-review-code` 等のスキルは今後実装予定。現時点ではルーティングの挙動を確認する。
...
**目的**: 「セキュリティ」「脆弱性」といったキーワードで、セキュリティ専門スキル（例: `river-review-security` ※今後実装予定）へルーティングされるか確認。
...
**目的**: 「設計」「アーキテクチャ」といったキーワードで、アーキテクチャ専門スキル（例: `river-review-architecture` ※今後実装予定）へルーティングされるか確認。
