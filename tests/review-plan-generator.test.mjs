...
import {
  ...
  resolveDepthToReviewMode,
  DEPTH_TO_REVIEW_MODE,
} from '../src/lib/review-plan-generator.mjs';

// --- manual depth override ---

test('determineReviewMode: manualMode overrides diff-size auto-detection', () => {
  assert.equal(
    determineReviewMode(
      { fileCount: 1, changedLines: 10, hasMigrations: false, hasSchemas: false },
      { manualMode: 'large' }
    ),
    'large'
  );
...
test('determineReviewMode: invalid manualMode falls back to auto-detection', () => {
  assert.equal(
    determineReviewMode(
      { fileCount: 1, changedLines: 10, hasMigrations: false, hasSchemas: false },
      { manualMode: 'bogus' }
...
test('resolveDepthToReviewMode: maps depth names to review modes', () => {
  assert.equal(resolveDepthToReviewMode('quick'), 'tiny');
  assert.equal(resolveDepthToReviewMode('standard'), 'medium');
  assert.equal(resolveDepthToReviewMode('thorough'), 'large');
});

test('resolveDepthToReviewMode: null/unknown yields null (auto)', () => {
  assert.equal(resolveDepthToReviewMode(null), null);
  assert.equal(resolveDepthToReviewMode(undefined), null);
  assert.equal(resolveDepthToReviewMode('exhaustive'), null);
  assert.deepEqual(Object.keys(DEPTH_TO_REVIEW_MODE), ['quick', 'standard', 'thorough']);
});
...
test('determineReviewMode: migration upgrades tiny to medium', () => {
  assert.equal(
    determineReviewMode({ fileCount: 1, changedLines: 10, hasMigrations: true, hasSchemas: false }),
    'medium'
  );
});
...
test('determineReviewMode: schema upgrades tiny to medium', () => {
  assert.equal(
    determineReviewMode({ fileCount: 1, changedLines: 5, hasMigrations: false, hasSchemas: true }),
    'medium'
  );
});
