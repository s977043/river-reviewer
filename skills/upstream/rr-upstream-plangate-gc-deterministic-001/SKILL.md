...
- 基準時刻 `now` は CLI から与えられる `--now` を使用し、未指定時は呼び出し時刻。決定論を保つため評価中は固定する。

### 4. Count（`reason: "count"` で REMOVE）

- Age 判定を通過した（= まだ KEEP 候補の）パスを scope 内で mtime 降順（新しい順）に並べ、`maxEntries` 番目を超えるエントリを REMOVE。
- 同一 mtime がある場合はタイブレーク規則（後述）で順序を一意化する。

### 5. Size（`reason: "size"` で REMOVE）

- Count 判定を通過したパスを scope 内で mtime 降順に並べ、cumulative `sizeBytes` が `maxSizeMb * 1024 * 1024` を超えた時点以降のエントリを REMOVE。

### 6. ルール衝突時の記録

- 同一パスが複数ルールに該当しても、`removed[]` には最初にヒットしたルール名のみを記録する。優先順は `age` > `count` > `size`（`cli-gc-spec.md` と同じ）。
...
- タイブレーク規則: mtime が同じエントリが複数ある場合は **path の昇順（lexicographic）** で古いものから並べる。
- 最終出力の `removed[]` は評価ロジックの順序に関わらず、**path の昇順（lexicographic）で再ソート**してから返す。これにより呼び出し順やハッシュマップ走査順に依存しない。
...
1. **Gate**: inventory が非空で各要素に `path` / `sizeBytes` / `mtimeEpoch` が揃い、retention config に `days` / `maxEntries` / `maxSizeMb` が数値で揃っているか確認。不成立なら `NO_REVIEW`。
...
4. **Apply count**: 残った KEEP 候補を mtime 降順 → path 昇順で並べ、`maxEntries` を超える分を REMOVE 候補に追加（`reason: "count"`）。
5. **Apply size**: さらに残った KEEP 候補を同じ順序で並べ、cumulative `sizeBytes` が `maxSizeMb * 1024 * 1024` を超えた時点以降を REMOVE 候補に追加（`reason: "size"`）。
6. **Deduplicate reason**: 複数ルールに該当したパスは `age > count > size` の優先順で 1 つだけ残す。
7. **Sort**: `removed[]` を path 昇順（lexicographic）で並べ替える。
8. **Assess severity**: Severity 節の条件を評価し、必要なら warning 行を出力。
9. **Output**: サマリ → 警告 → REMOVE 一覧の順で日本語出力。Human Handoff 条件に該当する場合は `questions` として返す。
